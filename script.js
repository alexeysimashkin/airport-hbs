...
// DATA STORAGE
// =================================================================
const STORAGE_KEY = 'koltsovo_flights_v2';
let nextId = 1;
let API_URL = '';
...
// ⚠️ ТОЛЬКО статус "Вылетел" считается вылетевшим!
...
async function saveFlightsToAPI(data) {
    try {
        const response = await fetch(`${API_URL}/flights`, {
            method: 'POST',
...
async function initFlights() {
    const loaded = await loadFlightsFromAPI();
    if (!loaded) {
        const localLoaded = loadFlightsFromLocal();
        if (!localLoaded) {
            flights = JSON.parse(JSON.stringify(DEFAULT_FLIGHTS));
            saveFlightsToAPI(flights);
            let maxId = 0;
            flights.forEach(f => { if (f.id > maxId) maxId = f.id; });
            nextId = maxId + 1;
        }
    }
    return flights;
...
// =================================================================
// ⚠️ АВТООБНОВЛЕНИЕ СТАТУСОВ (без перехода в "Вылетел")
// =================================================================
function updateFlightStatuses() {
    ...
    flights.forEach(f => {
        // Пропускаем рейсы, которые уже вылетели, задержаны или отменены
        if (f.status === 'Вылетел' || f.status === 'Задержан' || f.status === 'Отменён') return;

        let newStatus = f.status;

        // Автоматическое обновление (но НЕ ДО "Вылетел"!)
        if (f.registerStart && now >= f.registerStart && f.registerEnd && now < f.registerEnd) {
            ...
            // ⚠️ После окончания посадки — "Посадка закончена"
            // "Вылетел" ТОЛЬКО вручную админом!
            newStatus = 'Посадка закончена';
        }
        // ❌ НЕТ автоматического перехода в "Вылетел"
...
function renderBoard() {
    ...
    // ⚠️ ТОЛЬКО статус "Вылетел" скрывается кнопкой
    if (!showDeparted) {
...
function renderDetail(flightId) {
    ...
    const expTime = f.expectedTime ? formatTimeDisplay(f.expectedTime) : depTime;
    const expDate = f.expectedTime ? formatDateDisplay(f.expectedTime) : depDate;
